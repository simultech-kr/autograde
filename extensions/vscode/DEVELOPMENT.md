...
0.5.5: `bundle` 제출 전에 해당 과제의 미저장 문서만 확인·저장하며 저장 실패/취소 시 업로드하지 않습니다.
문서 범위는 제출 폴더 경로와 작업 영역 URI의 scheme/authority를 함께 검사해 WSL 원격 문서도 포함합니다.
빈 창에서는 폴더를 열기 전 수령 코드를 입력받거나 소비하지 않습니다.
접수 후에는 현재 `submission_id`로 약 3초 간격, 최대 10분 동안 비차단 조회합니다.
일시 연결 오류는 최대 3회 확인 후 쉬며, 결과 탭의 다시 확인으로 같은 접수번호를 재조회합니다.
`published`의 결과 응답이 잠시 준비되지 않은 경우 계속 확인하지만 `graded` 공개 대기는 멈춥니다.
로그아웃·주소/과제 변경·새 제출·이력 전환·탭 닫기는 이전 조회를 취소하고 늦은 응답을 무시합니다.
자동 확인 중지는 서버의 채점을 취소하지 않습니다. 과거 제출 화면은 정적 조회입니다.
스크립트 없는 결과 탭은 재확인/중지 두 명령만 허용하며 서버 문자열은 HTML escape합니다.

0.5.1: 다운로드 완료 후 현재 창의 탐색기에 과제 폴더를 표시하고
`main.cpp` → `main.c` → README 순으로 첫 안전한 파일을 자동으로 엽니다.
현재 로그인과 WSL 연결을 유지하기 위해 별도 창이나 workspace 전환은 하지 않습니다.
[동작 범위·현장 시험](../../docs/operations/download-open-workspace.md).

0.5.0: **채점 결과 요약 탭**에서 완료 판단, 큰 점수, 수정 필요 항목과 다음 할 일을 확인합니다.
공개 대기 결과는 점수를 표시하지 않으며, 과거 제출과 최신 제출을 구분합니다.
로그아웃·주소 변경 시 결과 탭을 닫습니다. [결과 화면·분반 관리 안내](../../docs/operations/results-and-course-admin.md).
...
`bundle` 다운로드는 새 VS Code window를 열거나 현재 workspace를 교체하지 않습니다.
현재 창의 탐색기에서 과제 폴더를 표시하고 안전한 최상위 `main.cpp`, `main.c`,
`README.md`, `README.txt`, `README` 순으로 첫 파일을 선택하여 editor에서 엽니다.
해당 파일이 없으면 폴더만 표시합니다. 따라서 Sign In → 다운로드 → 풀이 → 제출을 하나의
Extension Host와 로그인 세션에서 진행할 수 있습니다. 현재 workspace 자체가 이미 과제
폴더라면 다음 과제를 그 안에 중첩하지 않고, 상위 수업 폴더를 workspace로 열도록 안내합니다.
자동 열기가 실패해도 다운로드한 파일은 삭제하지 않으며 경로를 안내합니다.
...
관계없이 **내가 수락한 실습** 영역의 **수령 코드 입력 및 다운로드** 또는 제목 표시줄의 열쇠
아이콘을 사용할 수 있습니다. 기존 device 로그인은 호환 명령으로만 남아 있으며
학생 사이드바에서는 제공하지 않습니다. 로그인 후에는 과제를 펼쳐 **과제 파일 다운로드**를

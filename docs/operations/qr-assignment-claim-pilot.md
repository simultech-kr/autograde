...
- 학생이 입력하는 비밀번호는 학교 포털 비밀번호가 아니라 **Autograde 전용 비밀번호**다.
...
`pilot-local` 실행 중 하나라도 필요하면 **No-Go**입니다.
...
학생별로 15자 이상의 기억 가능한 Autograde 전용 passphrase를 등록합니다. 명령행 인자나
roster CSV로 비밀번호를 받지 않으며 화면에 두 번 입력합니다.
...
3. 학번과 Autograde 전용 비밀번호를 입력합니다.

# Autograde for Visual Studio — 개발·운영 참고

학생용 설치·사용 안내는 [확장 소개](README.md)를 먼저 읽으세요.
...
- 0.5.6: **학생 오류 복구와 제출 파일 검토 개선**. 미제출 과제는 정상 안내를 표시하고,
  자동 조회 중 인증이 만료되면 접수번호와 재로그인 안내를 유지하며 로그인 화면을 복원합니다.
  과제 전환 시 이전 다운로드 상태·문의번호를 지우고, 제출 확인창 안에서 전체 파일 목록을
  스크롤·복사한 뒤 제출하거나 취소할 수 있습니다. 기존 VSIX ID는 유지합니다.
  이번 변경의 C# 공통 검사와 컴파일은 로컬 .NET SDK 부재로 실행하지 못했으며,
  **Windows VSIX 생성·설치와 실제 화면 조작은 미검증입니다.**
- 0.5.1: **다운로드 후 과제 폴더 자동 열기**. 실패·취소 시 파일을 보존하며
  **다운로드한 과제 폴더 열기** 버튼으로 재시도합니다.
  [동작 범위·현장 시험](../../docs/operations/download-open-workspace.md).
...
7. 다운로드가 끝나면 받은 폴더가 VS에서 자동으로 열립니다. 기존 작업의 저장·신뢰 확인창은 직접 확인합니다.
   열기를 취소하거나 실패해도 파일은 보존되며 **다운로드한 과제 폴더 열기**로 재시도할 수 있습니다.
   확장은 빌드 명령을 실행하지 않지만 IDE 설정에 따른 CMake 자동 구성은 별도로 관리해야 합니다.
...
사용합니다. **0.5.4부터 접수를 확인한 뒤 새로 제출하면 내용이 같아도 새 요청 키와 접수번호를
발급합니다.** 서버도 파일 해시만으로 이전 접수를 재사용하지 않습니다. 확인되지 않은 요청 키만
최대 128개까지 메모리에 유지하며 접수 확인 시 제거합니다. 새 제출은 기존 일일 한도를 사용합니다.
...
학생 UX 회귀 확인 시에는 다음 조건도 확인합니다. 이 항목은 Windows IDE에서 직접 실행해야 합니다.

- 아직 제출하지 않은 과제에서 **최신 결과 확인**을 누르면 미제출 안내와 제출 방법이 나타나며 서버 오류나 점수로 표시하지 않습니다.
- 제출 후 자동 조회 중 세션을 만료시키면 로그인 입력이 즉시 나타나고 기존 접수번호와 재로그인 안내를 읽을 수 있습니다. 인증 갱신의 통신 실패로 세션이 사라진 경우도 동일합니다.
- A 과제의 다운로드 성공 또는 실패 후 B 과제를 선택하면 A의 준비 완료 문구, 오류 코드, 문의번호가 남지 않습니다.
- 긴 경로를 포함한 30개 이상의 파일을 제출할 때 확인창 안에서 마지막 파일까지 스크롤하고 목록을 복사할 수 있습니다. 창 크기 조절, Tab 이동, Esc 취소도 확인합니다. 취소 후 제출 기록이 늘어나지 않고, **목록 확인 후 제출**을 누른 경우에만 검토한 파일 묶음이 접수되어야 합니다.
...
| 이전 버전 → 0.5.1 설치·버전 표시 | 미검증 | 미검증 |
| 다운로드 후 폴더 자동 열기·기존 작업 저장 확인·취소 후 재시도 | 미검증 | 미검증 |

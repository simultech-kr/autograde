...
## 0.5.5 IDE 열기 복구 안내·확장 관리 설명

2026-09-17: 최신 Visual Studio 소스 버전은 **0.5.5**다. 아래 0.5.3/0.5.0 절은 해당 버전의
변경 기록이며 현재 배포 버전을 뜻하지 않는다. 기존 VSIX ID를 유지한다.

- 자동 폴더 열기 실패·취소를 다운로드 실패와 구분하고, 완료 요약 이후에도 저장 경로와
  **저장 → 과제 폴더 열기 재시도 → 파일 → 열기 → 폴더** 복구 순서를 표시한다.
- 폴더 열기 재시도는 파일을 다시 받지 않으며, 별도 진단 시도로 결과를 보고한다.
- 확장 관리 창의 설명은 `source.extension.vsixmanifest`에 반영했다. 시작 메뉴, API 주소와
  수령 코드, 필요 환경, 제출·결과·복원, 실패 복구와 공용 PC 안내를 포함한다.
- 이 변경은 Marketplace 게시나 설치된 확장의 즉시 갱신을 수행하지 않는다. Windows에서
  `extensions\visualstudio\build.ps1`로 VSIX를 생성·설치하고 확장 관리 창의 0.5.5 설명을 확인한다.

로컬 검증: Visual Studio 통합·계약 23개, 서버 다운로드 진단 21개, C# 공용 검사 126개 통과.
WPF compile-only 빌드 경고·오류 0개. 실제 Windows VSIX 설치·관리 창 표시·저장/신뢰 대화상자와
폴더 열기 실패 재현은 Windows 시험 PC에서 별도 확인해야 한다. 이번에는 설치용 VSIX를 생성하지 않았다.

[학생용 복구 절차](../../extensions/visualstudio/README.md#ide-열기-실패-안내-ag-dl-open-workspace)

## 0.5.3 버튼 정렬 수정

2026-09-17: 상단의 설정은 별도 Button, 로그아웃은 공통 AddButton으로 생성되어 서로 다른
위쪽 여백·안쪽 여백을 사용하던 문제를 수정했다. 헤더는 Grid로 정렬하고 모든 버튼에 같은
최소 높이·내용 정렬을 적용한다. 도구 모음 내부 폭 300 DIP 미만에서는 버튼 묶음을 아래 행으로
옮긴다. DPI 배율과 무관한 WPF 단위를 사용하며 고정 픽셀 높이로 텍스트를 잘라내지 않는다.

- 로그인 전: 설정·수령 코드 로그인만 사용 가능. 로그아웃·과제/폴더 관리 숨김, 결과·기록 탭 비활성.
- 로그인 후: 로그아웃·관리 항목·탭 복원. 다운로드·제출 버튼은 기존 과제 준비 상태에 따라 표시.
- 하단: 빈 메시지 상자는 숨기되 진행·오류·접수 메시지가 생기면 다시 표시.
- VS Code와 웹 UI 변경은 없으며 Visual Studio 버전만 0.5.3으로 올림. VSIX ID 유지.

저장소 루트에서 Windows PowerShell로 실행한다:

```powershell
.\extensions\visualstudio\build.ps1
```

생성된 `extensions\visualstudio\Autograde.VisualStudio\bin\Release\net472\Autograde.VisualStudio.vsix`를
설치하고 Visual Studio를 다시 연다. 화면에 **Autograde 0.5.3**이 표시되는지 확인한다.
다른 학생의 작업이 열려 있다면 먼저 저장한다. 기존 설치본을 제거해 설정을 초기화할 필요는 없다.

Windows에서 100%·125%·150% 배율, 밝음·어두움 테마, 폭 250·300·420 DIP로 확인한다.
설정/로그아웃 버튼의 상하 기준선 일치, 좁은 창의 줄 이동, 로그인·로그아웃 후 항목 표시,
오류 발생 시 하단 메시지 복원을 확인한다. macOS에서는 컴파일과 자동 계약 검사만 가능하며
실제 Windows 화면·VSIX 설치·업데이트 검증은 아직 수행하지 않았다.

검증: WPF 컴파일 경고·오류 0개, Visual Studio 관련 통합·계약 시험 22개 통과.
이번 작업에서는 설치용 Windows VSIX를 생성하거나 설치된 확장을 변경하지 않았다.

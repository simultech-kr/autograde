...
**과제 폴더 열기** 버튼으로 재시도할 수 있습니다.
재시도할 때는 폴더의 서버·과제 식별자가 현재 선택 과제와 맞는지 확인합니다.
VS Code는 실패 안내에 표시된 경로를 현재 탐색기에서 직접 열 수 있습니다.

### Visual Studio 0.5.5 복구 안내

`파일 준비 완료 · IDE 열기 실패`는 수락이나 다운로드 실패가 아닙니다. 재수락·재다운로드 없이
기존 작업을 저장하고 **과제 폴더 열기**를 재시도합니다. 실패·취소 안내에 저장 위치가 남습니다.
계속 실패하면 **파일 → 열기 → 폴더**에서 그 위치를 직접 엽니다. IDE의 저장·신뢰 확인창을
우회하거나 기존 코드를 삭제하지 않습니다.

**폴더 관리 · 다시 다운로드 → 제출 폴더**가 직접 연 과제 폴더와 같은지 반드시 확인합니다.
IDE의 현재 폴더와 확장의 제출 대상은 별개입니다. 재시작으로 로그인 정보가 사라졌다면 새 수령
코드로 로그인하고 **기존 과제 폴더 선택**을 사용합니다. 새 다운로드로 작업본을 대체하지 않습니다.

재시도 결과는 별도 진단 시도로 서버에 전달하며 실패 이력은 보존합니다. 수동 메뉴로 연 결과는
확장이 자동 확인하지 않으므로 서버의 이전 실패 기록만으로 현재 파일 손상을 단정하지 않습니다.
교수자에게는 문의번호·오류 코드·VS 버전·발생 상황을 전달하고 비밀번호·수령 코드는 보내지 않습니다.
로컬 저장 위치는 복구 화면에만 사용하며 서버 진단에 포함하지 않습니다.

[학생용 상세 절차](../../extensions/visualstudio/README.md#ide-열기-실패-안내-ag-dl-open-workspace)

...
2026-09-16 갱신. `python -m autograde.course_reset` 오프라인 도구 기준.
...
- 현재 스키마 10/11·독립 웹 초기화 완료·bundle 방식을 지원한다. 스키마 11에서는 웹에서
  등록한 새 수업도 정확한 `--course-key`로 선택할 수 있다. 아래 예시는 come2201이다.
  대상 수업의 CSV 등록 미리보기도 폐기하며 초안·검증 이력·다른 수업은 보존한다. 대상 수업에 Git 과제가 있거나

...
class CounterObserver final : public BoardObserver {
    int count_ = 0;
public:
    void onChange() override { /* TODO: record a change notification */ }
...
class ScoreIterator {
    ...
    int minimum_;
    void skip() {
        // TODO: skip entries below the minimum score.
    }
public:
    ScoreIterator(const std::vector<Submission>& rows, int minimum) : rows_(rows), minimum_(minimum) { skip(); }
    bool hasNext() const { /* TODO: determine iterator exhaustion */ return false; }
    const Submission& next() {
        // TODO: return the current entry and move to the next qualifying entry.
        return rows_.at(position_);
    }
};
class Board {
    std::vector<Submission> rows_;
    std::vector<std::shared_ptr<BoardObserver>> observers_;
    void notify() { /* TODO: notify each registered observer once */ }
...
public:
    ...
    void subscribe(std::shared_ptr<BoardObserver> observer) { (void)observer; /* TODO */ }
    void unsubscribe(const std::shared_ptr<BoardObserver>& observer) {
        (void)observer; // TODO: detach exactly this observer.
    }
    ...
    SnapshotPtr save() const {
        ...
        // TODO: save board entries, but not observers or their counts.
        return snapshot;
    }
    void restore(const Snapshot& snapshot) { (void)snapshot; /* TODO: restore and notify */ }
};
class BoardFacade {
    Board board_;
